...
OUTPUT_DIR = "llama3-finetuned-mascs"

wandb.login(key="wandb_key")

wandb.init(project="qlora-llama3-dialogue", name="llama3-8b-instruct-qlora-mascs", job_type="training")
...
prompt_text = load_prompt(f"mascs-dialogue-gen.txt")
...
def preprocess_mascs_function(examples):
    formatted_examples = []
...
dataset_mascs = load_dataset("json", data_files={"train":"datasets/mascs-train.json","test":"datasets/mascs-val.json"}, field="data")

# Shuffle Dataset
shuffled_dataset = DatasetDict({
    "train": dataset_mascs["train"].shuffle(seed=42),
    "test": dataset_mascs["test"].shuffle(seed=42)
})
